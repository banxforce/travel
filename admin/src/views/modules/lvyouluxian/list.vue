        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '路线标题' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.lvyouluxianName" placeholder="路线标题"
                            clearable></el-input>
                    </el-form-item>
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '路线类型' : ''">
                        <el-select v-model="searchForm.lvyouluxianTypes" placeholder="请选择路线类型">
                            ...
                            <el-option v-for="(item, index) in lvyouluxianTypesSelectSearch" v-bind:key="index"
                ...
                <el-row class="ad"
                    ...
                    <el-form-item>
                        <el-button v-if="isAuth('lvyouluxian', '新增')" type="success" icon="el-icon-plus"
                            @click="addOrUpdateHandler()">新增</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('lvyouluxian', '删除')" :disabled="dataListSelections.length <= 0"
                            type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
                        &nbsp;
                        <el-button v-if="isAuth('lvyouluxian', '报表')" type="success" icon="el-icon-pie-chart"
                            @click="chartDialog()">报表</el-button>
                        &nbsp;
                        <a style="text-decoration:none" class="el-button el-button--success"
                            v-if="isAuth('lvyouluxian', '导入导出')" icon="el-icon-download"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('lvyouluxian', '导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="lvyouluxian.xls">
                            <!-- 导出excel -->
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    v-if="isAuth('lvyouluxian', '查看')" :data="dataList" v-loading="dataListLoading"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="lvyouluxianUuidNumber" header-align="center" label="路线编号">
                        <template slot-scope="scope">
                            {{ scope.row.lvyouluxianUuidNumber }}
                        </template>
                    </el-table-column>

                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="lvyouluxianName" header-align="center" label="路线标题">
                        <template slot-scope="scope">
                            {{ scope.row.lvyouluxianName }}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="lvyouluxianTypes" header-align="center" label="路线类型">
                        <template slot-scope="scope">
                            {{ scope.row.lvyouluxianValue }}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                        prop="lvyouluxianPhoto" header-align="center" width="200" label="路线图片">
                        <template slot-scope="scope">
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="insertTime"
                        header-align="center" label="发布时间">
                        <template slot-scope="scope">
                    ...
                    <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('lvyouluxian', '查看')" type="success" icon="el-icon-tickets"
...
export default {
    data() {
        return {
            ...
            form: {
                id: null,
                lvyouluxianUuidNumber: null,
                lvyouluxianName: null,
                lvyouluxianTypes: null,
                lvyouluxianPhoto: null,
                lvyouluxianContent: null,
                insertTime: null,
                createTime: null,
            },
            dataList: [],
    ...
    methods: {
        chartDialog() {
            ...
            let params = {
                ...
                thisTable: {//当前表
                    tableName: "lvyouluxian",//当前表表名,
                    sumColum: 'lvyouluxian_number', //求和字段
                    date: 'insert_time',//分组日期字段
                    // string : 'lvyouluxian_name',//分组字符串字段
                    // types : 'lvyouluxian_types',//分组下拉框字段
                },
                // joinTable : {//级联表（可以不存在）
        ...
        getDataList() {
            ...
            params['lvyouluxianDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
        ...
        // 导入功能上传文件成功后调用导入方法
        lvyouluxianUploadSuccess(data) {
            let _this = this;
            _this.$http({
                url: "lvyouluxian/batchInsert?fileName=" + data.file,
                method: "get"
            }).then(({ data }) => {
                if (data && data.code === 0) {
                    _this.$message({
